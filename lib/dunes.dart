...
class ThirdDune extends CustomClipper<Path> {
  ...
  Path getClip(Size size){
    ...
    var firstEnd = Offset(size.width / 4.5,  1.47 * size.height / 2);
    path.cubicTo(firstControl.dx, firstControl.dy, secondControl.dx, secondControl.dy, firstEnd.dx, firstEnd.dy);

    var arcEnd = Offset(size.width / 2,  1.5 * size.height / 2);
    path.arcToPoint(arcEnd, radius: Radius.elliptical(size.width, 1.5 * size.height), clockwise: false);


    var thirdControl = Offset(size.width / 1.4,  1.35 * size.height / 2);
    var fourthControl = Offset(size.width / 1.5, 1.58 * size.height / 2);
    var secondEnd = Offset(size.width, 1.48 * size.height / 2);
    path.cubicTo(thirdControl.dx, thirdControl.dy, fourthControl.dx, fourthControl.dy, secondEnd.dx, secondEnd.dy);
...
class FourthDune extends CustomClipper<Path> {
  ...
  Path getClip(Size size){
    ...
    var firstControl = Offset(size.width / 4, 1.3 * size.height / 2);
    var secondControl = Offset(size.width / 4.5, 1.94 * size.height / 2);
    var firstEnd = Offset(size.width / 1.5,  1.57 * size.height / 2);
    path.cubicTo(firstControl.dx, firstControl.dy, secondControl.dx, secondControl.dy, firstEnd.dx, firstEnd.dy);

    var thirdControl = Offset(size.width / 1.2,  1.43 * size.height / 2);
    var secondEnd = Offset(size.width, 1.51 * size.height / 2);
    path.quadraticBezierTo(thirdControl.dx, thirdControl.dy, secondEnd.dx, secondEnd.dy);
